...
// static void autoTakeoffTask(void *arg);
// STATIC_MEM_TASK_ALLOC(autoTakeoffTask, 2048);
...
void systemStart()
{
 ...
 // STATIC_MEM_TASK_CREATE(autoTakeoffTask, autoTakeoffTask, "autoTakeoff", NULL, 5);
}

#define TAKEOFF_DELAY_MS  3000    // 起飞延迟（毫秒）- 等待传感器校准
#define TAKEOFF_THRUST    45000   // 起飞推力（0-65535）- 增大推力
#define HOVER_THRUST      38000   // 悬停推力 - 增大推力

static void autoTakeoffTask(void *arg)
{
  vTaskDelay(M2T(TAKEOFF_DELAY_MS));
  
  setpoint_t takeoffSetpoint = {0};
  takeoffSetpoint.thrust = TAKEOFF_THRUST;
  takeoffSetpoint.mode.roll = modeAbs;
  takeoffSetpoint.mode.pitch = modeAbs;
  takeoffSetpoint.mode.yaw = modeVelocity;
  takeoffSetpoint.mode.z = modeAbs;
  takeoffSetpoint.position.z = 0.5f;
  takeoffSetpoint.attitude.roll = 0;
  takeoffSetpoint.attitude.pitch = 0;
  takeoffSetpoint.attitudeRate.yaw = 0;
  
  commanderSetSetpoint(&takeoffSetpoint, COMMANDER_PRIORITY_CRTP);
  
  vTaskDelay(M2T(3000));
  
  takeoffSetpoint.thrust = HOVER_THRUST;
  commanderSetSetpoint(&takeoffSetpoint, COMMANDER_PRIORITY_CRTP);
