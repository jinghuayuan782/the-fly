...
static uint8_t wifi_channel = CONFIG_WIFI_CHANNEL;
static uint8_t wifi_max_sta_conn = CONFIG_WIFI_MAX_STA_CONN;
                               ...
                               int32_t event_id, void *event_data)
{
    if (event_id == WIFI_EVENT_AP_STACONNECTED) {
        ...
        DEBUG_PRINT_LOCAL("WiFi AP started");
    } else if (event_id == WIFI_EVENT_AP_STOP) {
        DEBUG_PRINT_LOCAL("WiFi AP stopped - THIS IS THE PROBLEM!");
    }
}
...
void wifiInit(void)
{
    ...
    ESP_ERROR_CHECK(espnow_ctrl_responder_bind(UINT32_MAX, -55, NULL));
